using MøbelProgram.Møbler;
using System;
using System.Collections.Generic;
using System.Text;

namespace MøbelProgram.Factories
{
    internal class Serie2Factory : IMøbelFactory
    {
        public IHjørneSofa CreateHjørneSofa()
        {
            return new Møbler.Serie2.HjørneSofa();
        }

        public ILænestol CreateLænestol()
        {
            return new Møbler.Serie2.Lænestol();
        }

        public ITaburet CreateTaburet()
        {
            return new Møbler.Serie2.Taburet();
        }

        public IToPersonerSofa CreateToPersonerSofa()
        {
            return new Møbler.Serie2.ToPersonerSofa();
        }

        public ITrePersonerSofa CreateTrePersonerSofa()
        {
            return new Møbler.Serie2.TrePersonerSofa();
        }
    }
}
